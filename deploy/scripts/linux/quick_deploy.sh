...
create_directories() {
    ...
    mkdir -p {data,artifacts/{cache,logs,models,reports},config,nginx/ssl,monitoring/{prometheus,grafana/{dashboards,datasources}}}

    # 设置权限
    chmod 755 {data,artifacts/{cache,logs,models,reports},config}
...
show_deployment_info() {
    ...
    echo "📁 数据目录:"
    echo "  数据文件: ./data/"
    echo "  缓存文件: ./artifacts/cache/"
    echo "  日志文件: ./artifacts/logs/"
    echo "  模型文件: ./artifacts/models/"
    echo "  输出文件: ./artifacts/reports/"
    echo ""
}
