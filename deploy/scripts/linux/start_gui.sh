...
if [ ! -f "frontend/streamlit/app.py" ]; then
    echo "❌ 找不到frontend/streamlit/app.py文件"
    exit 1
fi
...
if [ "$HOST" = "0.0.0.0" ]; then
    # 动态获取本机IP地址
    local_ip=$(python3 -c "import socket; s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM); s.connect(('8.8.8.8', 80)); ip = s.getsockname()[0]; s.close(); print(ip)" 2>/dev/null || echo "unknown")
    echo "🌐 GUI访问地址:"
