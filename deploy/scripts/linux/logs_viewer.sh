...
show_app_logs() {
    ...
    if [ -f "artifacts/logs/dlt_predictor.log" ]; then
        if [ "$follow" = "true" ]; then
            log_info "显示应用实时日志 (按Ctrl+C退出)"
            tail -f artifacts/logs/dlt_predictor.log
        else
            log_info "显示应用最近 $lines 行日志"
            tail -n "$lines" artifacts/logs/dlt_predictor.log
        fi
    else
        log_warning "应用日志文件不存在: artifacts/logs/dlt_predictor.log"
    fi
}
...
show_error_logs() {
    ...
    if [ -f "artifacts/logs/errors.log" ]; then
        log_info "显示错误日志最近 $lines 行"
        tail -n "$lines" artifacts/logs/errors.log
    else
        log_warning "错误日志文件不存在: artifacts/logs/errors.log"
    fi
}
...
show_dl_logs() {
    ...
    if [ -f "artifacts/logs/deep_learning.log" ]; then
        log_info "显示深度学习日志最近 $lines 行"
        tail -n "$lines" artifacts/logs/deep_learning.log
    else
        log_warning "深度学习日志文件不存在: artifacts/logs/deep_learning.log"
    fi
}
...
show_logs_overview() {
    ...
    if [ -d "artifacts/logs" ]; then
        ls -la artifacts/logs/ | grep -E "\.(log|txt)$" || echo "  无日志文件"
    else
        echo "  artifacts/logs目录不存在"
    fi
}
