...
try:
    from path_config import (
        CACHE_DIR, LOGS_DIR, MODELS_DIR, REPORTS_DIR,
        DATA_FILE,
        get_path_manager
    )
    ...
    try:
        ...
        CACHE_DIR = os.getenv('DLT_CACHE_DIR', "artifacts/cache")
        LOGS_DIR = os.getenv('DLT_LOGS_DIR', "artifacts/logs")
        MODELS_DIR = os.getenv('DLT_MODELS_DIR', "artifacts/models")
        REPORTS_DIR = os.getenv('DLT_REPORTS_DIR', "artifacts/reports")
        DATA_FILE = os.getenv('DLT_DATA_FILE', "data/dlt_data_all.csv")
...
class CacheManager:
    ...
    def __init__(self, cache_dir=None):
        # 使用统一的路径配置
        self.cache_dir = cache_dir if cache_dir else str(CACHE_DIR)
...
class LoggerManager:
    ...
    def __init__(self, log_dir=None, max_file_size=10*1024*1024, backup_count=5):
        # 使用统一的路径配置
        self.log_dir = log_dir if log_dir else str(LOGS_DIR)
...
class DataManager:
    ...
    def __init__(self, data_file=None, cache_manager=None):
        # 使用统一的路径配置
        self.data_file = data_file if data_file else DATA_FILE
    ...
    def get_period_range(self, start_period: int, end_period: int = None) -> pd.DataFrame:
        ...
        注意: 数据按期号降序排列（最新数据在第一行）
        start_period=0 表示从最新数据开始
        """
        if self.df is None:
            return pd.DataFrame()
    ...
    def get_latest_periods(self, count: int) -> pd.DataFrame:
        """获取最新的几期数据（数据按降序排列，最新在前）

        由于数据已按期号降序排列，最新数据在第一行，
        因此使用 head() 获取最新的 count 期数据
        """
        if self.df is None:
            return pd.DataFrame()
...
# 记录路径配置来源
logger_manager.debug(f"路径配置来源: {_PATH_CONFIG_SOURCE}")
if _PATH_CONFIG_SOURCE == "environment_variables_and_defaults":
    logger_manager.info(f"使用环境变量和默认路径配置 - CACHE_DIR: {CACHE_DIR}, LOGS_DIR: {LOGS_DIR}")
