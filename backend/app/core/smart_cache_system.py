...
try:
    from path_config import (
        CACHE_DIR, CACHE_ANALYSIS_DIR, CACHE_DATA_DIR, CACHE_MODELS_DIR,
        get_path_manager
    )
    ...
    try:
        ...
        CACHE_ANALYSIS_DIR = os.getenv('DLT_CACHE_ANALYSIS_DIR', "artifacts/cache/analysis")
        CACHE_DATA_DIR = os.getenv('DLT_CACHE_DATA_DIR', "artifacts/cache/data")
        CACHE_MODELS_DIR = os.getenv('DLT_CACHE_MODELS_DIR', "artifacts/cache/models")
        get_path_manager = None

# 在导入完成后记录路径配置来源
_logger = logging.getLogger('dlt_predictor')
_logger.debug(f"smart_cache_system 路径配置来源: {_PATH_CONFIG_SOURCE}")
if _PATH_CONFIG_SOURCE == "environment_variables_and_defaults":
    _logger.info(f"smart_cache_system 使用环境变量和默认路径配置 - CACHE_DIR: {CACHE_DIR}")


class SmartCacheManager:
    """智能缓存管理器 - 支持数据版本控制和自动失效"""
    
    def __init__(self, cache_dir=None, max_memory_cache_size=1000):
        # 使用统一路径配置，支持自定义覆盖
        self.cache_dir = cache_dir if cache_dir is not None else CACHE_DIR
        self.analysis_cache_dir = CACHE_ANALYSIS_DIR
        self.models_cache_dir = CACHE_MODELS_DIR
        self.data_cache_dir = CACHE_DATA_DIR
    ...
    def _get_data_signature(self) -> str:
        ...
        try:
            ...
            signature_data = {
                'total_rows': len(df),
                'latest_period': str(latest_row.get('issue', '')),
