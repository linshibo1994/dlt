...
class TransformerPredictor(BaseModel):
    ...
    def _transformer_predict_with_real_data(self, historical_data, count, periods=None):
        ...
        try:
            ...
            # 使用注意力机制分析历史数据
            recent_data = historical_data.head(150).iloc[::-1].reset_index(drop=True)  # 使用最近150期数据
        ...
        try:
            ...
            for i, row in data.iterrows():
                ...
                if len(front_balls) == 5 and len(back_balls) == 2:
                    ...
                    # 时序注意力分析（最近的数据权重更高）
                    weight = 1.0 / (len(data) - i)  # 越近的数据权重越高
...
class GANPredictor(BaseModel):
    ...
    def _prepare_gan_training_data(self, historical_data):
        ...
        try:
            ...
            # 限制数据量并重置索引
            data_to_process = historical_data.head(1000).iloc[::-1].reset_index(drop=True) if len(historical_data) > 1000 else historical_data.iloc[::-1].reset_index(drop=True)
    ...
    def _prepare_training_data_from_history(self, historical_data):
        ...
        try:
            ...
            for _, row in historical_data.head(1000).iloc[::-1].iterrows():
                front_balls = [int(x) for x in str(row.get('front_balls', '')).split(',') if x.strip().isdigit()]
                back_balls = [int(x) for x in str(row.get('back_balls', '')).split(',') if x.strip().isdigit()]
