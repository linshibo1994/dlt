...
from ..utils.interfaces import AnalyzerInterface
...
class AnomalyDetector(AnalyzerInterface):
        ...
        try:
            ...
            if method == 'zscore':
                ...
                for i, row in enumerate(data):
                    if np.any((row < lower_bound) | (row > upper_bound)):
                        # 计算异常分数
                        score = np.max(np.maximum(
                            (lower_bound - row) / (iqr + 1e-8),
                            (row - upper_bound) / (iqr + 1e-8)
                        ))
                        if score > 0:
                            anomalies.append(i)
                ...
                for i, scores in enumerate(modified_z_scores):
            ...
            logger_manager.info(f"统计异常检测完成: {method}, 发现 {len(anomalies)} 个异常")
            return result
    ...
    def detect_isolation_forest_anomalies(self, data: np.ndarray,
        ...
        try:
            ...
            scores = anomaly_scores[predictions == -1].tolist()
            ...
            logger_manager.info(f"孤立森林异常检测完成: 发现 {len(anomalies)} 个异常")
            return result

        except ImportError:
            logger_manager.error("sklearn未安装，无法使用孤立森林")
            return {}
        except Exception as e:
            logger_manager.error(f"孤立森林异常检测失败: {e}")
            return {}

    def detect_local_outlier_factor_anomalies(self, data: np.ndarray,
                                            n_neighbors: int = 20,
                                            contamination: float = 0.1) -> Dict[str, Any]:
        """
        局部异常因子检测
        ...
        try:
            ...
            logger_manager.info(f"LOF异常检测完成: 发现 {len(anomalies)} 个异常")
            return result

        except ImportError:
            logger_manager.error("sklearn未安装，无法使用LOF")
            return {}
        except Exception as e:
            logger_manager.error(f"LOF异常检测失败: {e}")
            return {}
