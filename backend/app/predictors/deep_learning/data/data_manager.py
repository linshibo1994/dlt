...
class DeepLearningDataManager:
    ...
    def augment_data(self, progress_bar, features: np.ndarray, factor: float = None) -> np.ndarray:
        ...
        for i in range(new_samples_count):
            ...
            # 使用正弦函数生成确定性噪声
            noise = np.sin(np.linspace(0, 2*np.pi, len(sample)) + i * 0.1) * noise_level

            # 确保号码特征（前7个）在增强后仍然有效
            augmented_sample = sample + noise
            # 数据增强边界值裁剪说明：
            # - 特征顺序为 [front_1, front_2, front_3, front_4, front_5, back_1, back_2]
            # - 前5个是前区号码，有效范围 1-35
            # - 后2个是后区号码，有效范围 1-12
