...
def check_gui_interface():
    ...
    try:
        # 检查GUI文件是否存在
        gui_file = os.path.join(PROJECT_ROOT, "gui_app.py")
        if os.path.exists(gui_file):
            ...
            try:
                ...
                print(f"  ERROR GUI模块导入失败: {str(e)[:100]}...")
                return False
        else:
            print("  ERROR GUI文件不存在")
            return False

    except Exception as e:
        print(f"  ERROR GUI检查异常: {str(e)}")
        return False
