...
def show_prediction_page():
    ...
    with tab2:
        ...
        with col1:
            if st.button("🎯 九模型预测", use_container_width=True, key="deep_nine_models"):
                with st.spinner("正在进行九模型预测..."):
                    try:
                        predictor = AdvancedPredictor()
                        result = predictor.nine_models_predict(count=count, periods=periods)
...
def show_compound_prediction_page():
    ...
    with col1:
        if st.button("🎯 频率分析复式", use_container_width=True, key="compound_freq_new"):
    ...
    with col2:
        if st.button("🔥 冷热分析复式", use_container_width=True, key="compound_hot_cold_new"):
    ...
    with col3:
        if st.button("📉 遗漏分析复式", use_container_width=True, key="compound_missing_new"):
    ...
    with col1:
        if st.button("🎲 1阶马尔可夫复式", use_container_width=True, key="compound_markov_1st_new"):
    ...
    with col2:
        if st.button("🎯 2阶马尔可夫复式", use_container_width=True, key="compound_markov_2nd_new"):
    ...
    with col3:
        if st.button("🎯 3阶马尔可夫复式", use_container_width=True, key="compound_markov_3rd_new"):
    ...
    with col4:
        if st.button("🔄 自适应马尔可夫复式", use_container_width=True, key="compound_adaptive_markov_new"):
    ...
    with col1:
        if st.button("📊 贝叶斯复式", use_container_width=True, key="compound_bayesian_new"):
    ...
    with col2:
        if st.button("🔗 集成复式", use_container_width=True, key="compound_ensemble_new"):
    ...
    with col3:
        if st.button("🧮 九模型复式", use_container_width=True, key="compound_nine_new"):
    ...
    if dl_available:
        ...
        with col1:
            if st.button("🧠 LSTM复式", use_container_width=True, key="compound_lstm_new"):
                    ...
                    try:
                        ...
                        if lstm_model:
                            ...
                            if hasattr(lstm_model, 'predict_compound'):
                                ...
                                # 回退实现：基于LSTM预测结果生成复式
                                base_result = lstm_predictor.predict(data)
    ...
    with col1:
        if st.button("🚀 超级复式", use_container_width=True, key="compound_super_new"):
    ...
    with col2:
        if st.button("🔄 自适应复式", use_container_width=True, key="compound_adaptive_new"):
