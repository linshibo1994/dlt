...
class DLTAdvancedAnalyzer:
    ...
    def predict_by_ensemble(self, explain=False, num_draws=1):
        ...
        try:
            ...
            if explain:
                print(f"马尔可夫链预测: 前区 {','.join([str(b).zfill(2) for b in front_balls])}, 后区 {','.join([str(b).zfill(2) for b in back_balls])}")
        except Exception as e:
            print(f"马尔可夫链预测失败: {e}")
        ...
        try:
            ...
            if explain:
                print(f"贝叶斯预测: 前区 {','.join([str(b).zfill(2) for b in front_balls])}, 后区 {','.join([str(b).zfill(2) for b in back_balls])}")
        except Exception as e:
            print(f"贝叶斯预测失败: {e}")
        ...
        try:
            ...
            if explain:
                print(f"频率预测: 前区 {','.join([str(b).zfill(2) for b in front_balls])}, 后区 {','.join([str(b).zfill(2) for b in back_balls])}")
        except Exception as e:
            print(f"频率预测失败: {e}")
        ...
        try:
            ...
            if explain:
                print(f"统计预测: 前区 {','.join([str(b).zfill(2) for b in front_balls])}, 后区 {','.join([str(b).zfill(2) for b in back_balls])}")
        except Exception as e:
            print(f"统计预测失败: {e}")
        ...
        if explain:
            ...
            print("投票详情:")
            print(f"  前区投票: {dict(sorted_front[:10])}")
            print(f"  后区投票: {dict(sorted_back[:5])}")
    ...
    def _predict_by_statistics(self):
        ...
        while attempts < max_attempts:
            ...
            if abs(sum(front_balls) - target_sum) <= 20:  # 允许一定偏差
                break
            attempts += 1
