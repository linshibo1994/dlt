...
class DataAugmentor:
    ...
    def generate_synthetic_data(self, count: int, feature_dim: int,
        ...
        # 注意：数据增强器应该基于真实数据进行增强，而不是生成完全随机的数据
        # 这里需要基于历史数据的统计特征来生成合成数据
        ...
        if historical_data is not None and len(historical_data) > 0:
            # 基于真实数据的统计特征生成合成数据
            synthetic_data = self._generate_based_on_real_data(historical_data, count, feature_dim, method)
        else:
            # 如果没有真实数据，使用固定的合理分布
            logger_manager.warning("没有真实数据，使用固定分布生成合成数据")
            if method == 'gaussian':
                # 使用固定的高斯分布参数
                synthetic_data = np.full((count, feature_dim), 0.5)
                # 添加小的固定变化
                for i in range(count):
                    for j in range(feature_dim):
                        synthetic_data[i, j] += (i * 0.01 + j * 0.001) % 0.3 - 0.15
                synthetic_data = np.clip(synthetic_data, 0, 1)
            else:
                # 使用固定的均匀分布
                synthetic_data = np.linspace(0.1, 0.9, count * feature_dim).reshape(count, feature_dim)
        
        logger_manager.info(f"合成数据生成完成，方法: {method}，数据形状: {synthetic_data.shape}")
        
        return synthetic_data

    def _generate_based_on_real_data(self, historical_data, count, feature_dim, method):
        """基于真实历史数据生成合成数据"""
        try:
            import pandas as pd
            ...
            for _, row in historical_data.head(1000).iterrows():
                ...
                if len(front_balls) == 5 and len(back_balls) == 2:
                    ...
                    feature = front_normalized + back_normalized
                    # 如果需要更多特征，可以添加统计特征
                    while len(feature) < feature_dim:
                        feature.append(sum(feature) / len(feature))  # 添加平均值特征

                    real_features.append(feature[:feature_dim])

            if not real_features:
                # 如果没有有效的真实特征，返回固定分布
                return np.linspace(0.1, 0.9, count * feature_dim).reshape(count, feature_dim)
            ...
            if method == 'gaussian':
                ...
                synthetic_data = []
                for i in range(count):
                    # 基于真实数据的统计特征生成，但使用确定性方法而不是随机
                    sample = mean + std * np.sin(np.linspace(0, 2*np.pi, feature_dim) + i * 0.1)
                ...
                # 基于真实数据的范围生成均匀分布
                min_vals = np.min(real_features, axis=0)
                max_vals = np.max(real_features, axis=0)

                synthetic_data = []
                for i in range(count):
                    # 在真实数据的范围内生成确定性分布
                    sample = min_vals + (max_vals - min_vals) * (i / count)
                    synthetic_data.append(sample)

                return np.array(synthetic_data)

        except Exception as e:
            logger_manager.error(f"基于真实数据生成合成数据失败: {e}")
            # 返回固定分布作为后备
            return np.linspace(0.1, 0.9, count * feature_dim).reshape(count, feature_dim)
