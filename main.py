...
from dlt_500_crawler import DLT500Crawler
...
def crawl(args):
    ...
    try:
        ...
        # 获取历史数据
        results = crawler.get_history_data(count=100)
...
def main():
    ...
    # 集成预测子命令
    ensemble_parser = subparsers.add_parser("ensemble", help="使用集成方法预测")
    ensemble_parser.add_argument("-d", "--data-file", default="../data/dlt_data.csv", help="数据文件路径")
    ensemble_parser.add_argument("-c", "--count", type=int, default=5, help="生成预测号码注数")
    ensemble_parser.add_argument("--explain", action="store_true", help="解释预测结果")
    ensemble_parser.add_argument("--check-latest", action="store_true", help="检查与最新一期的匹配情况")
